...
SAX also accepts opt-in Sla handlers in component files. Existing SA label handlers such as `@inc: L_ENTRY: ...` are still supported; Sla handlers use normal function syntax and are compiled through `sa_plugin_sla`. Component-local Sla imports such as `@import "helpers.sla"` are passed through to the handler bridge. Components that use Sla handlers automatically release their state during destroy, so they do not need explicit `!state_name` release lines:
